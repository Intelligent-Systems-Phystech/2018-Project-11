...
def get_experiment_dir(exp_num):
    return "experiments/exp{exp_num}_alternation/run1/".format(exp_num=exp_num)

exp_dir = get_experiment_dir(4) #look in experiments dir and set proper exp_num in order not to rewrite existing folder
if not os.path.exists(exp_dir):
    os.makedirs(exp_dir)
    
args.save = 'search-{}-{}'.format(args.save, time.strftime("%Y%m%d-%H%M%S"))
utils.create_exp_dir(os.path.join(exp_dir, args.save), scripts_to_save=glob.glob('*.py'))

log_format = '%(asctime)s %(message)s'
logging.basicConfig(stream=sys.stdout, level=logging.INFO,
    format=log_format, datefmt='%m/%d %I:%M:%S %p')
fh = logging.FileHandler(os.path.join(exp_dir, args.save, 'log.txt'))
...
def main():
  ...
  model = Network(args.init_channels, CIFAR_CLASSES, args.layers, criterion, 0.05, 1)
  model = model.cuda()
  ...
  for epoch in range(args.epochs):
    ...
    model_epoch_dir = (exp_dir + "/epoch{epoch_num}/").format(epoch_num=epoch)
...
def train(train_queue, valid_queue, model, architect, criterion, optimizer, lr):
  ...
  #experiment4 (variant) in the beginning of epoch - weak, in the end - hard
  #start_temp = 0.1
  #end_temp = 10e-30
  #temp_step = (start_temp - end_temp)/400

  #cur_temp = start_temp + temp_step
